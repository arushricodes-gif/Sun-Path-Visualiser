...
with tab1:
    ...
    folium.TileLayer(
        ...
        attr='&copy; OpenStreetMap contributors', name='Street'
    ).add_to(m)
...
with tab_summary:
    st.markdown(
        '<div class="theory-section"><h2 class="theory-header">Seasonal Comparison For Selected Location</h2></div>',
        unsafe_allow_html=True
    )
    ...
    for ms in milestones:
        ...
        c = m_r
        while c <= m_s:
            lat_p, lon_p, _, _, _, _ = solarlogic.get_solar_pos(
                city_info, c, radius_meters, lat, lon
            )
